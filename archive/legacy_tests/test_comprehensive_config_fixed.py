...
class TestComprehensiveConfig(unittest.TestCase):
    ...
    def test_basic_calculation(self):
        ...
        self.assertIn("final_charge", result)
        self.assertIn("total_duration_minutes", result)
        self.assertGreaterEqual(result["final_charge"], 0)

        print(f"基本計算測試通過: {result['final_charge']}元")
    ...
    def test_vehicle_type_differences(self):
        ...
        for vehicle_type in vehicle_types:
            if "universal_mall" in self.calculator.rate_plans:
                ...
                results[vehicle_type.value] = result["final_charge"]
    ...
    def test_discount_application(self):
        ...
        if "universal_mall" in self.calculator.rate_plans:
            ...
            self.assertIn("discounts_applied", result)
            print(f"優惠測試結果: {result['final_charge']}元")
            if result.get("discounts_applied"):
                print(f"已套用優惠: {result['discounts_applied']}")
    ...
    def test_daily_cap_functionality(self):
        ...
        print(
            f"每日上限測試結果: {result['final_charge']}元，停車{result['total_duration_minutes']/60:.1f}小時"
        )
    ...
    def test_grace_period_functionality(self):
        ...
        print(
            f"寬限期測試結果: 停車{result['total_duration_minutes']}分鐘，費用{result['final_charge']}元"
        )
    ...
    def test_progressive_rates(self):
        ...
        if "mall_complex" in self.calculator.rate_plans:
            ...
            print(f"累進費率測試結果: {result['final_charge']}元")
    ...
    def test_cross_day_parking(self):
        ...
        print(f"跨日停車測試結果: {result['final_charge']}元")
    ...
    def test_weekend_vs_weekday_rates(self):
        ...
        print(f"平日費率: {weekday_result['final_charge']}元")
        print(f"假日費率: {weekend_result['final_charge']}元")
    ...
    def test_manual_adjustment(self):
        ...
        expected_amount = base_result["final_charge"] + 50
        self.assertEqual(adjusted_result["final_charge"], expected_amount)

        print(
            f"手動調整測試: 基礎{base_result['final_charge']}元 -> 調整後{adjusted_result['final_charge']}元"
        )
